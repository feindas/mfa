...
! elefante
!         call fix_VCM()
! la ultima linea no deberia ser necesaria 
...
#      if WALL  == 1 || WALL == 2 /* if using or explicit implicit wall with fixed brush heads */
...
#if SYSTEM == 4
! elefante
! To calculate the temperature wihtout the beads of the rings
! por que calcular la temp sin considerar los anillos?
      ...
      t_fluid = T_inst
!Facundo check T      print *,"T_inst= ",T_inst/(3.*dble(n_mon*n_chain))
...
!deb facu print*, " v_wall_wall, v_fluid_fluid, v_fluid_wall", v_wall_wall, v_fluid_fluid, v_fluid_wall
!deb facu stop
!
...
! Be carefully!, e_total is t_total + v_total, significa que t_total debe normalizarse 
!antes de calcular el e_total (lo mismo con el v_total)
!        write(60,'(i7,3g17.5)') i_time,v_total*inv_N, e_total*inv_N,t_total*inv_N
        write(60,'(i7,3g17.5)') i_time,v_total*inv_N, ( v_total*inv_N + t_total/(dble(n_mon*n_chain)) ), t_total/(dble(n_mon*n_chain))
#elif SYSTEM == 2  || SYSTEM == 3
        v_total = v_total + v_coul
        write(60,'(i7,4g17.5)') i_time,v_total*inv_N, e_total*inv_N,t_total*inv_N,v_coul*inv_N
#endif
