...
!! obs !
!! obs !---  Initialize temperature
!! obs !
!! obs               if(temp_time.eq.0.) then
!! obs               temp = temp_final
!! obs               else
!! obs               temp = temp_init
!! obs               end if
...
#   if CHAIN_BC == 1        
#         if CHAIN_TYPE ==0  /* Chain with regular positions in X axis */
           call gen_chain(0) 
#         elif CHAIN_TYPE ==1  /* Chain with harmonic positions in X axis */
!           print '(/a/)'," * conf_default harmonic chain entrada!!"
           call gen_chain(1)
!           print '(/a/)'," * conf_default harmonic chain salida!!"
  !         call write_conf(2,r0(:,:),32)
#         endif 
#   endif /* CHAIN_BC = 1 ; periodic boundary conditions */

#   if CHAIN_BC == 2        /* Fix-ends boundary conditions */
           call gen_chain(2)
#   endif 
             ! write out r0 and check !
#if RINGS != 0
           call gen_ring(n_ring)
#endif
!  ------ the new ring + chain is stored in file: new_system
!open(unit=10, file = 'new_system.xyz', status = 'new', action = 'write', &
!        position = 'append')
!
!        write(10,* ) n_mon*n_chain+n_mon_d*n_chain_d+n_mon_e*n_chain_e
!        write(10,* ) 
!        do i_part=1,n_mon*n_chain!+n_mon*n_chain_d+n_mon_e*n_chain_e
!                write( 10, * ) " N ",  r0(1,i_part), r0(2,i_part), r0(3,i_part)
!        end do
!!        write(10,*) n_mon_d
!!        write(10,*) 
!        do i_part=n_mon*n_chain+1,n_mon*n_chain + n_mon_d*n_chain_d
!                write( 10, * ) " O ",  r0(1,i_part), r0(2,i_part), r0(3,i_part)
!        end do
!        do i_part=n_mon*n_chain+n_mon_d*n_chain_d+1,n_mon*n_chain+n_mon_d*n_chain_d+n_mon_e*n_chain_e
!                write( 10, * ) " C ",  r0(1,i_part), r0(2,i_part), r0(3,i_part)
!        end do
!
!close(unit=10)
...
#   endif
        ...
        call intra_molec()
#if SYSTEM == 4
! In the first configuration does not store forces over rings
        call fix_force_CM(2) ! fixing the rings
#endif
...
!
    ...
    do i_part = 1,part_init_d
      ...
      vec_dummy(:) = vec_dummy(:)/n_mon
	print'(a,3g16.7)', "[conf_default] S4 - Fcm_cadena (por bead):", vec_dummy(:)

       vec_dummy(:) = 0. 
       r_dummy = 0.
    do i_part = 1,part_init_d
         vec_dummy(:) = vec_dummy(:) + mass(i_part)*a(:,i_part)
         r_dummy = r_dummy + mass(i_part)
    end do
      vec_dummy(:) = vec_dummy(:)/r_dummy
	print'(a,3g16.7)', "[conf_default] S4 - Acm_cadena :", vec_dummy(:)
      vec_dummy(:) = 0.
      r_dummy = 0.
      do i_part = 1 , n_mon
      ...
      vec_dummy(:) = vec_dummy(:)/r_dummy
      print'(a,3f16.5)', "[conf_default] S4 - Vcm (chain-debug)=", vec_dummy(:) 
      vec_dummy(:) = 0.
      r_dummy = 0.
	print'(a//)', "*************************************************************"
#endif
