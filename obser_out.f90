...
close(235)
    ...
    open(unit=24, file='parametros_fft', position ='append')
        write(24,*) "[chain] n_beads_chain: ", n_mon
        write(24,*) "[ring] n_ring: ", n_ring
        write(24,*) "[ring] ring_distance: ", ring_dist
        write(24,*) "[ring] n_beads_ring1: ", n_mon_d
        write(24,*) "[ring] n_beads_ring2: ", n_mon_e
    close(24)

! deb elefante fr1 fr2
!          print '(a,3f16.8)','FORCE ring 1 = ',fr1(:)/dble(count_obs)
!          print '(a,3f16.8)','FORCE ring 2 = ',fr2(:)/dble(count_obs)
          print '(a,3f16.8)','FORCE CM CHAIN = ',fr_CM(:)/dble(count_CM_force)
          print '(a,i4)','COUNT_CM_FORCE = ',count_CM_force

!          print '(a,3f16.8)','VEL ring 1 = ',vr1(:)/dble(count_obs)
!          print '(a,3f16.8)','VEL ring 2 = ',vr2(:)/dble(count_obs)

!         print *,'random=',randi/rand_c

!the next close if SYSTEM == 4
#endif
