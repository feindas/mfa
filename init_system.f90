...
! *  reads in system_input file the system  size and characteristics 
...
#if SYSTEM == 4
#     if RINGS == 0
        read(14,*) n_mon,n_chain,n_cell_w_x,n_cell_w_y, n_box_fftw, n_safe_fftw !  
#     else
        read(14,*) n_mon,n_chain,n_cell_w_x,n_cell_w_y, n_box_fftw, n_safe_fftw, n_ring, ring_dist, ring_radio, rfijo !  
#     endif
...
!      z_skin ~ minimum distance between bottom and wall in z coodinate in which the melt will be generated
        ...
        if(n_read/=0) then 
            print *," WARN!! missing parameter in system_input. Line for particle 4! "
           stop 
          endif
#endif
...
#if SYSTEM == 4 /* chain plus rings */
        print '(/a)', "   ------- Chain + Rings Parameters --------"
        print '(a,2i5)', "  * num of chain, chain beads = ", n_chain, n_mon 
#    if RINGS != 0
        print '(a,i5,f10.5)', "  * number of rings, rings distance = ", n_ring, ring_dist
          print '(a,2i5)', "  * ring 1  n_mon_d, n_chain_d = ",n_mon_d, n_chain_d 
          print '(a,2i5)', "  * ring 2  n_mon_e, n_chain_e = ",n_mon_e, n_chain_e 
        if(rfijo == 0) then
          print '(a,2i5)', " WARNING! The rings position Y & Z are fixed!! rfijo=", rfijo
        else if(rfijo == 1) then
          print '(a,2i5)', " WARNING! The rings position X, Y & Z are fixed!! rfijo=", rfijo
        else if(rfijo == 2) then
          print '(a,2i5)', " WARNING! The rings are free, rfijo=", rfijo
        endif
#    endif
#    ifdef PARTICLE_4
        print *, " * n_mon_e, n_chain_e, inter4 = ",n_mon_e, n_chain_e, inter4
#       endif
        print '(/a)', "        -------- FFTW  Parameters --------"
        print '(a,i6)', "  * n_box_fftw = ", n_box_fftw
...
#endif
#if STORE == 0 && SYSTEM == 4   /* if I/O is done with unfolded coordinates  */
      allocate (r0_unfold(n_dim,n_part))
#endif
...
!RINGS FIXED CONDITION IN intra_molec   allocate ( pos_ini_rings( 3, (n_mon_d*n_chain_d+n_mon_e*n_chain_e ) ) )
   if( n_ring .ne. 0 ) then
      allocate ( total_ring_force(3, n_ring), total_ring_force2(3,n_ring) )
      allocate ( ring_force(3, n_ring) )
      allocate ( ring_force_2( 3, n_ring ) )
   else 
      allocate ( total_ring_force(3, 1), total_ring_force2(3,1) )
      allocate ( ring_force(3, 1) )
      allocate ( ring_force_2( 3, 1 ) )
   endif
     total_ring_force(:,:)= 0.
     total_ring_force2(:,:)= 0.
     ring_force(:,:)= 0.
