...
!---  bond distance       
       real (kind=8) ,allocatable :: bond_dist(:) !cummulative distance between two consecutive chain monomers

       real (kind=8) ,allocatable :: add_bond_dist(:) !cummulative value for bond distance. dim 2. 1: bonds outside the rings
																								!  2: bonds between the rings
       real (kind=8) ,allocatable :: mean_bond_dist(:) !mean value for bond distance. dim 2. 1: bonds outside the rings
																					    !    2: bonds between the rings																								

       real (kind=8) ,allocatable :: error_bond_dist(:) !error in the mean distance between two consecutive chain monomers

       real (kind=8) ,allocatable :: bond_dist2(:) !cummulative cuadratic distance between two consecutive chain monomers
      
       real (kind=8) ,allocatable :: add_bond_dist2(:) !cuadratic cummulative value for bond distance. dim 2. 1: bonds outside the rings
																											! 2: bonds between the rings
       real (kind=8) ,allocatable :: mean_bond_dist2(:) !cuadratic mean value for bond distance. dim 2. 1: bonds outside the rings
																									!   2: bonds between the rings
...
!     n_cell_w_x,n_cell_w_y : number of cells for wall in x,y direction
      ...
      integer :: i_wall,i_part,j_part !,n_part,n_layer !,n_wall !ori
      integer :: n_safe
...
#    if RINGS != 0
real (kind =8), allocatable ::  ring_force(:,:), ring_force_2(:,:)
real (kind =8), allocatable ::  total_ring_force(:,:), total_ring_force2(:,:)
! FIXED RINGS implemented in intra_molec real (kind =8), allocatable ::  pos_ini_rings(:,:)
!integer :: n_safe_ring_force=10
real (kind =8) :: inv_n_safe, inv_n_obser,k_ring
#    endif
#       if RINGS == 0
real (kind =8) :: k_ring
#       endif
...
real (kind=8) :: fr_CM(3) ! force CM chain
real (kind =8) :: ring_dist, ring_radio 
! this variable is necesary for gen_ring (in all configuration of SYSTEM == 4)
! thats the problem of global variable for compile. ( think about new Makefile? )
real (kind =8) :: q_fftw, delta_x
complex (kind =8), allocatable :: Ipar_fftw(:)
integer :: n_box_fftw=1000, n_screen
integer :: n_ring = 0. , rfijo = 0
integer (kind=8) :: count_CM_force = 0
integer (kind=8) :: plan_fftw, j_fftw = 0, i_fftw
    ! this variables was for subroutine create_chain_heigt() in chain_fftw.f90
real(kind = 8) :: h2pprom = 0., h2pdesv = 0.
real(kind = 8) :: hprom = 0., hprom2 = 0.
...
! rf1,rf2: debug
!     real (kind=8) :: fr1(3),fr2(3) ! for debug
