...
!          end if  ! explicit wall atoms     
          ...
          print '(a/)', "        Configuration file reading... Done!"
...
!      else !if not explicit wall atoms
#       else    
          rx_twall(:,:) =0.0
#       endif
!      end if !epxlicit wall

!
!!obsolete!---- reset ramp for temperature
!!obsolete
!!obsolete      temp = temp_init
!
...
!
!! obsolete       if(f_minimize.eq.1) then
!! obsolete !---   freeze the system
!! obsolete           do i_order = 1,n_order
!! obsolete               do i_dim = 1,n_dim
!! obsolete                   do i_part = 1,n_part
!! obsolete                       rx(i_part,i_dim,i_order) = 0.
!! obsolete                   end do
!! obsolete                   rx_twall(i_dim,i_order) = 0.
!! obsolete               end do
!! obsolete           end do

!obsolete !---   chose a different friction constant
!obsolete           do i_type = 1,n_type
!obsolete               friction(i_type) = 10.
!obsolete           end do
!obsolete !---   release the top wall
!obsolete           f_twall(2) = 2
!obsolete           va_spring_twall(2) = 0.
!obsolete       end if
...
#if SYSTEM == 2 ||  SYSTEM==3 /* Coulomb or Bulk simulations SYSTEM 3 is added by Leo */ 
#       if SYMMETRY == 1 
...
! elefante
      ...
      vec_dummy(:) = vec_dummy(:)/r_dummy
      print '(a,3f16.5)',"[init_config] S4 - First Fcm(per bead) (chain)=", vec_dummy(:) 
      vec_dummy(:) = 0.
      r_dummy = 0.
      do i_part = 1 , n_mon*n_chain
      ...
      vec_dummy(:) = vec_dummy(:)/r_dummy
      print '(a,3f16.5)',"[init_config] S4 - First Acm (chain)=", vec_dummy(:) 
      vec_dummy(:) = 0.
      r_dummy = 0.
      do i_part = 1 , n_mon*n_chain
          vec_dummy(:) = vec_dummy(:) + mass(i_part)*v(:,i_part)
      ...
      vec_dummy(:) = vec_dummy(:)/r_dummy
      print'(a,3f16.5)', "[init_config] S4 - Vcm (chain)=", vec_dummy(:) 
