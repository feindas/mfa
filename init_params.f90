...
#include "control_simulation.h"
      implicit none
!obs      integer, parameter ::  n_order_max=5
      real (kind=8) :: init_dens,graft_dens,wall_t_area
!obs      real (kind=8)         :: pred_coef(0:n_order_max-1,0:n_order_max),corr_coef(0:n_order_max)
...
!    end if
    ...
    read(10,*) dt              ;print '(a30,f12.5)',"   dt          ",dt                
    read(10,*) c_dummy         ! line
    read(10,*) !r_dummy       !;print '(a30,g10.5)',"   temp_time   ",temp_time                   
    read(10,*) temp          ;print '(a30,f10.5)',"   Temperature   ",temp                  
    read(10,*) !r_dummy      !;print '(a30,g10.5)',"   temp_final  ",temp_final           
    read(10,*) !c_dummy         ! line
    read(10,*) x_space         ;  print '(a30,f12.5)' ,"   x_space     ",x_space
    read(10,*) y_space         ;  print '(a30,f12.5)' ,"   y_space     ",y_space               
    read(10,*) z_space_wall    ;  print '(a30,f12.5)' ,"   z_space_wall ",z_space_wall          
    read(10,*) !c_dummy         !;print*,"c_dummy     ",c_dummy           

! ---- Read interaction parameters 

    do i_type = 1,n_type  ! Reading loop 
        read(10,*) mass_type(i_type) ; print*,      "    mass_type = ", mass_type(i_type)
        do j_type=1,n_type 
            read(10,*) epsil(i_type,j_type) ; print *,  "    epsil(i,j) =", epsil(i_type,j_type)
        end do
        read(10,* ) sigma(i_type,i_type)  ; print *,"  sigma(i,i) =", sigma(i_type,i_type) 
        read(10,*) friction(i_type),c_dummy ; print *,"friction=" ,friction(i_type)
    end do
...
# endif
    ...
    read(10,*)! c_dummy
    read(10,*)! c_dummy
    read(10,*) (f_twall(i_dim),i_dim=1,n_dim) ; print '(a,3i6)', "    f_twall =",f_twall(:)
    read(10,*) (va_spring_twall(i_dim),i_dim=1,n_dim) ; print '(a,3f10.5)', "    Wall velocity = ",va_spring_twall(:)
#ifndef SHEARED
    read(10,*) (k_spring_twall(i_dim),i_dim=1,n_dim) ;print '(a,3f10.5)', "    k_spring_twall(:) = ",k_spring_twall(:)
#else
    read(10,*) (turn_time(i_dim),i_dim=1,n_dim) ; print '(a,3f10.5)',"    turn_time=" , turn_time(:)
    k_spring_twall(:) = 0.0
    read(10, *) p_time
...
!
   ...
   !grafting density [1/sigma^2]
      ...
      write(20,'(f18.5,a50)') init_dens, "      initial monomer density "
...
# if SYSTEM == 0 & WALL !=1
print *, "******************************************
         * WARNING! dont you need epsil(1,1) = 0.  *
         *******************************************"
         stop
#       endif             
...
#    endif
        print *,'  *  Interaction with implicit walls:' 
        print '(a,4(f8.3,x))','   sigma_w= ',sigma_wall(:)
        print '(a,4(f8.3,x))','   a_w    = ',a_wall
!        print '(a,4(f5.3,x))','   a_w    = ',a_wall(:)
#endif
...
#   if SYSTEM == 0 || SYSTEM == 3 
         if(mod(n_chain,2) == 0 ) then 
           ...
           print '(/a/)', "WARNING: n_chain is not divisible by 2 ! "
           stop
         end if
#   endif

#if SYSTEM == 4 
! Definition of force constant for the rings      
        if (rfijo == 1 ) then
            k_ring = k_chain 
        else if (rfijo == 0)  then
        ! criterium (1/2)*k_spring*<x^2> = (1/2)*10kT i, using <x^2>~1  then    
            k_ring = 5.0*temp*r_chain_2 
        end if
      print '(a,g16.6)'," * Force constant for ring springs: k_ring= ",k_ring
